type ScanFramingGuideProps = {
  /** Shown inside the frame, telling the user what to do. */
  instruction: string;
  /** Secondary line, shown under the frame. */
  hint: string;
};

/**
 * Framing assistant drawn over the camera preview.
 *
 * The camera image is rendered by the operating system behind a transparent
 * webview, so everything here is deliberately see-through except the frame
 * itself: the dimmed surround is a single large shadow cast outwards by the
 * cutout.
 */
